...
func TestRenderer_PostgresAlterIdentityColumnTypeOmitsIdentityClause(t *testing.T) {
	renderer, err := NewRenderer("postgres", "public", "fail")
	if err != nil {
		t.Fatalf("NewRenderer: %v", err)
	}

	got, err := renderer.AlterColumnTypeDDL("Orders", driver.Column{
		Name:       "ID",
		DataType:   "bigint",
		IsNullable: false,
		IsIdentity: true,
	...
	if err != nil {
		t.Fatalf("AlterColumnTypeDDL: %v", err)
	}

	assertEqualSQL(t, got, `ALTER TABLE "public"."orders" ALTER COLUMN "id" TYPE bigint`)
	if strings.Contains(got, "IDENTITY") {
		t.Fatalf("ALTER COLUMN TYPE must not include identity clause, got %q", got)
	}
}
