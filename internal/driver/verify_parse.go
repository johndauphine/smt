  ...
  "columns": [
    {
      ...
      "datetime_precision": <integer; fractional-seconds digits for date/time/timestamp types — the N in TIMESTAMP(N)/DATETIME2(N)/DATETIMEOFFSET(N)/TIME(N); 0 if none>,
...
`)
	...
	sb.WriteString("9. datetime_precision: the fractional-seconds precision of a temporal type — the N in `TIMESTAMP(N)`, `DATETIME2(N)`, `DATETIMEOFFSET(N)`, `TIME(N)`, `timestamp(N) with|without time zone`. Use 0 when no precision is written. This is SEPARATE from precision/scale, which are for decimal/numeric/money only.\n")
