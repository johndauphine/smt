...
// connected PostgreSQL server's version. Three tiers:
//
//   - PG 12+ (modern):  is_identity AND is_generated/generation_expression
//     both available → catches `GENERATED AS IDENTITY` and `GENERATED ALWAYS
//     AS (...) STORED`.
//   - PG 10 / 11 (mid): is_identity available, but is_generated and
//     generation_expression do not exist on information_schema.columns
//     until PG 12. Reference them and the query errors with
//     `column does not exist`. Use the modern identity gate but stub out
//     the generated-column fields.
//   - PG 9.x (legacy):  neither is_identity nor is_generated; fall back to
//     the `column_default LIKE 'nextval%'` heuristic that catches
//     SERIAL/BIGSERIAL/SMALLSERIAL. PG 9.x predates GENERATED AS IDENTITY
//     and stored generated columns, so the heuristic is exhaustive.
...
func (r *Reader) loadColumns(ctx context.Context, t *driver.Table) error {
	...
	for rows.Next() {
		...
		// SERIAL / BIGSERIAL columns set IsIdentity=true via the nextval(...) heuristic
		// in loadColumnsSQL. The sequence call also lands in DefaultExpression; emitting
		// both as `... GENERATED BY DEFAULT AS IDENTITY DEFAULT nextval(...)` confuses
		// the AI and produces invalid DDL. The IDENTITY annotation is the load-bearing
		// part — drop the duplicated default.
		if c.IsIdentity && strings.HasPrefix(c.DefaultExpression, "nextval(") {
			c.DefaultExpression = ""
		}
