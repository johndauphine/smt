...
// TestBuildSourceDDLWithTarget covers the per-column rendering paths added
// alongside the DefaultExpression / IsComputed / ComputedPersisted column
// metadata. The AI prompt depends on these strings being correct — silent
// regressions here surface as missing NOT NULL / DEFAULT / generated columns
// in target DDL (see PR #9 for the bug this prompt structure was built to fix).
func TestBuildSourceDDLWithTarget(t *testing.T) {
	m := &AITypeMapper{}
	...
	tests := []struct {
		...
		col         Column
		sourceDB    string
		wantContain []string // substrings expected in the rendered line
	...
	}{
		{
			...
			sourceDB:    "mssql",
			wantContain: []string{"is_active bit NOT NULL DEFAULT ((1))"},
		},
		{
			...
			sourceDB:    "mssql",
			wantContain: []string{"created_at datetime2 NOT NULL DEFAULT (getutcdate())"},
		},
		{
			name: "MSSQL IDENTITY — no default emitted",
			col: Column{
				Name: "id", DataType: "int", Precision: 10, IsNullable: false,
				IsIdentity: true,
			},
			sourceDB:    "mssql",
			wantContain: []string{"id int(10) NOT NULL IDENTITY"},
			notContain:  []string{"DEFAULT"}, // IDENTITY columns shouldn't carry a separate DEFAULT
		},
		{
			name: "PG identity from sequence — DefaultExpression must be cleared by reader so prompt is clean",
			// Reader is responsible for clearing nextval(...) when IsIdentity is set;
			// this test asserts the prompt path doesn't double-emit when that happens.
			col: Column{
				Name: "id", DataType: "int4", IsNullable: false,
				IsIdentity:        true,
				DefaultExpression: "", // cleared by PG reader
			},
			sourceDB:    "postgres",
			wantContain: []string{"id int4 NOT NULL GENERATED BY DEFAULT AS IDENTITY"},
			notContain:  []string{"DEFAULT nextval", "DEFAULT  GENERATED"},
		},
		{
			name: "computed STORED with explicit type (PG/MySQL style)",
			col: Column{
			...
			sourceDB:    "postgres",
			wantContain: []string{"full_name text AS (first_name || ' ' || last_name) STORED"},
			notContain:  []string{"VIRTUAL"},
		},
		{
			...
			sourceDB:    "mysql",
			wantContain: []string{"margin decimal(8,4) AS ((unit_price - cost_price) / cost_price) VIRTUAL"},
			notContain:  []string{"STORED"},
		},
		{
			name: "computed PERSISTED with no type (MSSQL — type inferred)",
			col: Column{
				Name: "line_total", DataType: "", // MSSQL sometimes omits inferred type
				IsComputed: true, ComputedExpression: "quantity * unit_price",
				ComputedPersisted: true,
			},
			sourceDB:    "mssql",
			wantContain: []string{"line_total AS (quantity * unit_price) STORED"},
			notContain:  []string{"line_total  AS"}, // no double-space if type empty
		},
		{
			name: "nullable column with no default and no NOT NULL",
			col: Column{
				Name: "phone", DataType: "varchar", MaxLength: 50, IsNullable: true,
			},
			sourceDB:    "mssql",
			wantContain: []string{"phone varchar(50)"},
			notContain:  []string{"NOT NULL", "DEFAULT"},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got := m.buildSourceDDLWithTarget(tbl, tt.sourceDB, "")
			for _, s := range tt.wantContain {
				if !strings.Contains(got, s) {
					t.Errorf("buildSourceDDLWithTarget output missing %q\n--- got ---\n%s", s, got)
				}
			}
			for _, s := range tt.notContain {
				if strings.Contains(got, s) {
					t.Errorf("buildSourceDDLWithTarget output unexpectedly contains %q\n--- got ---\n%s", s, got)
				}
			}
		})
	}
