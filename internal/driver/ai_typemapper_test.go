...
// TestBuildTableDDLPrompt_PreviousAttempt is the regression test for the
// validate-and-retry prompt augmentation (see #29). When
// TableDDLRequest.PreviousAttempt is non-nil, the prompt MUST include a
// "PRIOR ATTEMPT FAILED" section containing the verbatim prior DDL and the
// verbatim database error — that's the corrective context that converts
// gpt-oss-20b's deterministic-failure cells into successful retries (variance
// experiment showed ~81% per-call success at temp=1; with retries that
// becomes ~99.9% per-table).
//
// When PreviousAttempt is nil (the first-try / no-retry case), the section
// MUST be absent — otherwise every prompt would carry a confusing empty
// retry section.
func TestBuildTableDDLPrompt_PreviousAttempt(t *testing.T) {
	mapper := testMapperWithTempCache(t, "anthropic", testProvider("test-key"))
	...
	const failedDDL = `CREATE TABLE dbo.OrderItems (id int NOT NULL, line_total decimal(34,11) AS (qty * unit_price) PERSISTED);`
	const dbError = "mssql: Incorrect syntax near the keyword 'AS'."

	tests := []struct {
		name       string
		req        TableDDLRequest
		wantPhrase []string // phrases that MUST be in the prompt
		notPhrase  []string // phrases that MUST NOT be in the prompt
	}{
		{
			name: "no PreviousAttempt — section omitted",
			req: TableDDLRequest{
				SourceDBType: "mssql", TargetDBType: "mssql",
				SourceTable: tbl, TargetSchema: "dbo",
			},
			notPhrase: []string{"PRIOR ATTEMPT FAILED", "Previous DDL", "Database error"},
		},
		{
			name: "with PreviousAttempt — section present with verbatim DDL + error",
			req: TableDDLRequest{
				SourceDBType: "mssql", TargetDBType: "mssql",
				SourceTable: tbl, TargetSchema: "dbo",
			...
			wantPhrase: []string{
				"=== PRIOR ATTEMPT FAILED ===",
				"Previous DDL (verbatim):",
				failedDDL, // the actual prior DDL must appear verbatim
				"Database error (verbatim):",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			prompt := mapper.buildTableDDLPrompt(tt.req)
			for _, p := range tt.wantPhrase {
				if !strings.Contains(prompt, p) {
					t.Errorf("prompt missing required phrase %q\n--- prompt tail ---\n%s",
						p, prompt[max(0, len(prompt)-1000):])
				}
			}
			for _, p := range tt.notPhrase {
				if strings.Contains(prompt, p) {
					t.Errorf("prompt unexpectedly contains phrase %q", p)
				}
			}
		})
	}
...
// migration of the same table-shape would hit the cached failure again. The
// writer calls CacheTableDDL after the database confirms the DDL is valid;
// future GenerateTableDDL calls (no PreviousAttempt) should then return
// the validated DDL from cache rather than re-calling the AI.
func TestCacheTableDDL(t *testing.T) {
	mapper := testMapperWithTempCache(t, "anthropic", testProvider("test-key"))
	...
	req := TableDDLRequest{
		...
		SourceTable: tbl, TargetSchema: "public",
	}
	const validatedDDL = "CREATE TABLE public.companies (id integer NOT NULL GENERATED BY DEFAULT AS IDENTITY, PRIMARY KEY (id));"

	mapper.CacheTableDDL(req, validatedDDL)

	// Now exercise the cache-hit path on a first-try lookup (PreviousAttempt = nil).
	// The cache key is built deterministically from req in tableCacheKey; if
	// CacheTableDDL stored under a different key shape, this lookup misses.
	cacheKey := mapper.tableCacheKey(req)
	mapper.cacheMu.RLock()
	cached, ok := mapper.cache.Get(cacheKey)
	mapper.cacheMu.RUnlock()
	if !ok {
		t.Fatal("expected cache hit after CacheTableDDL but got miss")
	}
	if cached != validatedDDL {
		t.Errorf("cached DDL mismatch:\n  got:  %q\n  want: %q", cached, validatedDDL)
	}

	// CacheTableDDL must replace, not append. Calling again with a different
	// DDL for the same key must overwrite — otherwise a successful retry can't
	// fully clean up a previously-cached bad DDL.
	const replacementDDL = "CREATE TABLE public.companies (id integer NOT NULL GENERATED BY DEFAULT AS IDENTITY);"
	mapper.CacheTableDDL(req, replacementDDL)
	mapper.cacheMu.RLock()
	cached, _ = mapper.cache.Get(cacheKey)
	mapper.cacheMu.RUnlock()
	if cached != replacementDDL {
		t.Errorf("CacheTableDDL did not replace prior value:\n  got:  %q\n  want: %q", cached, replacementDDL)
	}
}
