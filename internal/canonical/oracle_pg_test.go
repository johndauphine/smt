...
func corpus() []tcase {
	...
	add(mk("mysql", "float", canonical.TypeMeta{}))
	add(mk("mysql", "double", canonical.TypeMeta{}))
	add(mk("mysql", "real", canonical.TypeMeta{}))
	add(mk("postgres", "double precision", canonical.TypeMeta{}))
	add(mk("postgres", "real", canonical.TypeMeta{}))
	add(mk("mssql", "nchar", canonical.TypeMeta{MaxLength: 10}))
	add(mk("mysql", "set", canonical.TypeMeta{EnumValues: []string{"x", "y"}}))
	add(mk("mysql", "blob", canonical.TypeMeta{}))
	add(mk("mysql", "tinyblob", canonical.TypeMeta{}))
	add(mk("postgres", "time with time zone", canonical.TypeMeta{}))
	add(mk("mysql", "timestamp", canonical.TypeMeta{DatetimePrecision: ip(6)}))
	// character
