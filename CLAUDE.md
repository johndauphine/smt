...
- **Type mapping and DDL generation** for supported source/target pairs through `internal/ddl.Renderer` and per-driver deterministic helpers. Type mapping goes through the canonical type IR (`internal/canonical`, #62): `ToCanonical(typeName, TypeMeta, sourceDialect)` normalizes a source column's dialect-specific type into a dialect-neutral `CanonicalType`, and `FromCanonical(ct, targetDialect, RenderOpts)` renders it to the target's DDL type. All three renderer targets (mssql/mysql in `internal/ddl/renderer.go`, postgres in `internal/driver/postgres/deterministic.go`) route through this one pipeline, so they share one source→canonical→target mapping. `canonical` is a leaf package taking primitives (`canonical.TypeMeta`, built via `driver.MetaOf`), not `driver.Column`, so `package driver` can import it without a cycle. Unknown/non-portable types surface as `Kind: Raw` → `ErrUnknownType` → the renderer's unknown-type policy. `RendererVersion` (in `internal/ddl`) bumps whenever this mapping can change output for the same input.
- **DEFAULT / CHECK expression translation** through the expression IR (`internal/expr`, #175), modeled on the canonical type IR: `ParseDefault` / `ParseCheck` parse a source expression once into a ~11-node dialect-neutral tree (literals, semantic function categories like `NowDateTime`/`UtcNow`/`UuidGen`/`Coalesce`/`Concat`, comparisons, `IN`, LIKE/regex, class-relevant date/time casts), `Render` emits it for any of the three targets with equal coverage, and `Equal`/`ClassLabel` compare structurally (this is what `driver.DefaultExpressionsEquivalent` and criterion 6's `defaultExpressionClass` run on). Like `canonical`, `expr` is a leaf package (callers supply `QuoteIdent` and `ColInfo` context), so `package driver` imports it without a cycle. Forms outside the grammar parse to `Raw`; `Render` refuses them (`ErrUnsupported`) and the caller falls back to the legacy per-target string-rewrite pipeline (still used for computed columns and index filters), which ends in the shared fail-closed gate `expr.RejectUnknownFunctions` — on every target, an untranslatable function errors rather than passing through. Bump `RendererVersion` when translation output can change.
...
`ai_review.enabled: true` adds an AI **parse + deterministic-compare** pass that inspects the already-rendered deterministic DDL before apply. It does **not** author or rewrite executable DDL — review is advisory (`ai_review.mode: warn`) or a hard gate (`mode: fail`), never a generator. The reviewer model parses the proposed target DDL into structured `Column[]` JSON; Go-side `CompareColumns` (`internal/driver/verify_compare.go`) runs the per-column criteria mechanically (max_length, precision/scale, nullability, identity, TZ class, default class) against the source and surfaces any deltas as `Issues`. Pair a cheap parser with a stronger one via `ai_review.model` (a provider entry in the secrets file). The legacy `migration.ai_verify` / `ai_verifier_model` aliases are removed from the v1 config contract; stale configs must rename them to `ai_review.enabled` / `ai_review.model`.
...
Scope note: the deterministic comparator covers TABLE DDL and finalization side objects. CREATE INDEX / FOREIGN KEY / CHECK CONSTRAINT review parses SMT's generated DDL with narrow Go parsers and compares source metadata deterministically; CHECK and filtered-index predicates use `internal/expr` structural comparison when the predicate is inside that grammar, with normalized text as the fallback.
...
- **#62 (PRs #116, #117, #118, #119)** — Canonical type IR. Type mapping moved from per-target dialect-string→dialect-string switches onto a shared source→canonical→target pipeline (`internal/canonical`: `CanonicalType` + `ToCanonical` + `FromCanonical`), modeled on UVG's `ddl_typemap`. All three renderer targets route through it. Clean-model deltas adopted along the way (the "normalize to the more-correct mapping" decision): MySQL `tinyint(1)`→`BIT`/`boolean` (with pg default `1`→`true` and `IN (0,1)`→`IN (false,true)` CHECK rewrites via the generalized `rewriteBooleanComparisons`), dialect-correct float/real precision (MySQL FLOAT=single, REAL=double-synonym), and pg now accepts MySQL `mediumint` / `time with time zone` sources that the old switch rejected. `driver.MetaOf` is the single `Column→TypeMeta` builder; `RendererVersion` bumped 1→2. Remaining #62 ACs (spatial `Kind`, lossy-conversion warnings) and the optional drift/`CompareColumns` comparator-unification are tracked on the issue as follow-ups — the comparator still uses its own hand-tuned equivalence rules (`verify_compare.go`) and was re-validated post-migration (no drift false-positives).
...
- **#67 (PR #110), #153** — `migration.ai_verify` / `ai_verifier_model` were accepted as deprecated 0.x aliases for `ai_review.*`; for v1 they are removed and stale configs fail with rename guidance. Drive-by in #67: `logging.SetOutput(nil)` resets to the default instead of panicking on the next write.
