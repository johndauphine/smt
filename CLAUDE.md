...
`migration.ai_verify: true` adds an AI self-check pass between DDL generation and exec. The auditor receives the source introspection block plus the proposed DDL and reports OK or ISSUES against the same six criteria the harness applies. Verifier-flagged issues are fed back into the next generation attempt as `PreviousAttempt`, sharing the `ai_max_retries` budget with exec-fail retries. Cache hits skip verify (cached DDL was already verified and executed). By default, generation and verification use the same provider (Phase 1); set `migration.ai_verifier_model` to a different provider entry in the secrets file to pair a cheap/local generator with a strong cloud auditor (Phase 2 — cross-model verify; the canonical workaround for sub-20B local generators where same-model verify hits correlated-bias false positives). Opt-in default — set the flag to enable. To force re-verification of cached entries after enabling, clear `~/.smt/type-cache.json`.
