...
| `gemma-4-e4b-it-mlx` (MLX, ~4B effective) | 2 | **FAIL** — auditor false-positives on the prompt's ACCEPTABLE list (`((1)) ≡ 0/1` for PG bit→boolean defaults, `IDENTITY ≡ GENERATED BY DEFAULT AS IDENTITY`, `GETUTCDATE() ≡ CURRENT_TIMESTAMP`); the generator "corrects" by dropping a real default to silence the auditor. Net: introduces a regression worse than verify-off. |
