...
-- the 14 CRM tables don't exercise (issue #46). Lengths stay at or below
-- 8000 so every target can keep them inline (MSSQL's inline ceiling).
-- TIMETZ is deliberately omitted: MSSQL and MySQL have no tz-aware
-- time-of-day type, so any mapping would be a by-design tz-class failure.
CREATE TABLE type_smoke (
    ...
    ts_p0           TIMESTAMP(0)   NOT NULL,
    ts_p3           TIMESTAMP(3),
    tstz_p2         TIMESTAMPTZ(2),
