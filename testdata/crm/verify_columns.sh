...
awk -F'|' 'NF>=8 {gsub(/[ \t]+$/, "", $0); print}' "$work/src.tsv" | sort > "$work/src.sorted"
awk -F'|' 'NF>=8 {gsub(/[ \t]+$/, "", $0); print}' "$work/tgt.tsv" | sort > "$work/tgt.sorted"
...
BEGIN {
  ...
  for (i = 1; i <= src_n; i++) {
    ...
    if (s_dt[key] ~ /^(datetime2|datetimeoffset)$/ && s_dt[key] != t_dt[key]) {
      printf("  FAIL %s tz-awareness: source=%s target=%s\n", key, s_dt[key], t_dt[key]); fail++
    }
  ...
  printf("== verify_columns: PASS (all criteria 1–5 met across %d columns) ==\n", src_n)
}'
